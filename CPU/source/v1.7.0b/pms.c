...
void pms_scan_advanced()
{
  ...
  int32 freq1, freq2;
  int8 d_type, d_seg, curr_vfo_band;
  int8 curr_channel;
  int8 pms_state = state;
  if ((hard_500k) || (soft_500k)) pms_state +=4;
  int8 count = 0;
  int8 old_region = 0;
  int8 old_channel = 0;
  int1 cb_upd = 0;
  ...
  int1 blink = 0;
  while(sw_pms){}
  curr_vfo_band = calc_band(storage_buffer[state]);
   ...
   if(pms_state == 3)
   {
      ...
      IF (freq1 < freq2) temp_freq = freq1;
      IF (freq1 > freq2) temp_freq = freq2;
   ...
   if((pms_state == 4) || (pms_state == 8))
   {
      ...
      d_type = 2;
      temp_freq = load_cb(cb_channel, cb_region);
      flash_en = 1;
   }
#endif  
   if(pms_state == 7)
   ...
   while(true)
   {
      ...
      switch(d_type)
      {
         ...
         switch(blink)
         {
            case 1: d_seg = state; break;
            case 0: d_seg = 0xFF; break;
         }
         VFD_data (d_seg, 0xFF, temp_freq, curr_channel, 0) ;
         break;
         
         case 2:
         switch(blink)
         {
            case 1:  d_seg = 3; break;
            case 0:  d_seg = 0xFF; break;
         }
         VFD_data (d_seg, 0xFF, curr_channel, 15, 2) ;
         break;
      }
      ...
      if(!stopped)
      {
         switch(pms_state)
         {
            ...
            if(dir) //freq1 is lower than freq2
            {
               ...
               if(temp_freq > freq2) temp_freq = freq1; else temp_freq += 2;
            }
...
#ifdef include_cb
      if(cb_upd) {temp_freq = load_cb(curr_channel, cb_region); cb_upd = 0;}
#endif
      if(mr_upd) {temp_freq = load_mem_ch_f(curr_channel); mr_upd = 0;}
   ...
   {
      if(state == 4) {cb_channel = old_channel; cb_region = old_region; temp_freq = load_cb(cb_channel, cb_region);}
      if(state == 3) {mem_channel = old_channel; temp_freq = load_mem_ch_f(mem_channel);}
      storage_buffer[state] = storage_buffer[0];
      storage_buffer[0] = 0;
   }
}
