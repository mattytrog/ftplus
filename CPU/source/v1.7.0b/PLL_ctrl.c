...
int8 old_PORTA = 0;
void Q64_rest(INT8 val)
{
         
         switch(val)
         {
            default: if(valid) PORTA = 0; else PORTA = 6; break;
            case 1: PORTA = 10; break;
            case 2: PORTA = 14; tx_timer_start = 1; break;
...
const int16 blacklist_start[10]= 
{
   15,
   35,
   70,
...
//if PLL update is requested for the SAME frequency, you are sent on your way and bounced back whence you came lol
      ...
      if(!setup_offset)
      {
         if((!tx_offset_only) && (!rx_offset_only)) offset = 0;
         else
         {
            temp_offset = load_offset_f();
            if(tx_offset_only)
            {
               if(otx) offset = temp_offset; else offset = 0;
            }
            if(rx_offset_only)
            {
               if(otx) offset = 0; else offset = temp_offset;
            }
            if((tx_offset_only) && (rx_offset_only)) offset = temp_offset;
      ...
      //printf(" %ld ", offset_frequency);
      while(offset_frequency > 99999)     {offset_frequency -= 100000;  tmp_band_freq += 10;}
      while(offset_frequency > 9999)      {offset_frequency -= 10000;   tmp_band_freq +=1;   tmp_khz_freq += 100;}
      while(offset_frequency > 999)       {offset_frequency -= 1000;    tmp_khz_freq += 10;}
      while(offset_frequency > 99)        {offset_frequency -= 100;     tmp_khz_freq += 1;}
      d100h = 0;  while(offset_frequency > 9)         {offset_frequency -= 10;      d100h+=1;}
      d10h = 0;   while(offset_frequency > 0)         {offset_frequency -= 1;       d10h+=1;}
      ...
      old_khz_freq = tmp_khz_freq;
         ...
         IF(!gen_tx)
         {
            for (INT i = 0; i < 10; i++)
               {
                  valid = 0;
                  IF ( (tmp_band_freq >= blacklist_start[i]) && (tmp_band_freq <= blacklist_end[i])) {valid = 1; break;}
               }
