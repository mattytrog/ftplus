...
int32 join_value(int8 d3, int8 d4, int8 d5, int8 d6, int8 d7, int8 d8, int8 d9)
{
   int32 v = 0;
   while(d9 > 0) {v += 1; d9 -= 1;}
   while(d8 > 0) {v += 10; d8 -= 1;}
   while(d7 > 0) {v += 100; d7 -= 1;}
   while(d6 > 0) {v += 1000; d6 -= 1;}
   while(d5 > 0) {v += 10000; d5 -= 1;}
   while(d4 > 0) {v += 100000; d4 -= 1;}
   while(d3 > 0) {v += 1000000; d3 -= 1;}
   return v;
}
...
void manual_adjust_frequency()
   {
      ...
      while(true)
      {
         ...
         res = buttons(1);
         if(res) {general_timer_start = 1; general_timer = 0; disp_flash = 0;}
         if(general_timer >= 3) {disp_flash = flash; general_timer_start = 0;}
...
void program_offset(int16 offset_val)
{
   ...
   setup_offset = 1;
   if((!tx_offset_only) && (!rx_offset_only)) {write8(txrx_offset_only_n, 3); tx_offset_only = 1; rx_offset_only = 1;}
   ...
   while(true)
   {
      ...
      IF (res == 1)
      {
         ...
         set_PLL(temp_freq, 1,0xFF);
         
         res = 0;
      }
      if(offset_val) {delay_ms(1000); res = 2;}
      if(!res)
      {
...
void LONG_up()
{
   IF((state == 1) || (state == 2) || (state == 4)) {IF(mem_channel  < 14)++mem_channel; write8(mem_ch_n, mem_channel); }
...
void LONG_dn()
{
   IF((state == 1) || (state == 2) || (state == 4)) {IF(mem_channel  > 0)--mem_channel; write8(mem_ch_n, mem_channel);}
...
void toggle_cb_mode()
{
   ...
   if((gen_tx) && (!cb_disabled))
   {
      ...
      switch (state)
      {
         case 4: save_cb_state(); state = old_state; cl = 0; write8(state_n, state); break;
