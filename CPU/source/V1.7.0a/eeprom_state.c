...
{
   if (state == 1)                                {write32 ((vfoa_block + band), storage_buffer[1]);}
   if (state == 2)                                {write32 ((vfob_block + band), storage_buffer[2]);}
}
...
{
   int8 offset_loc = 0;
   if(per_band_offset) offset_loc = (offset_block + PLLband + 1); else offset_loc = offset_block;
   write32 (offset_loc, data);
}
