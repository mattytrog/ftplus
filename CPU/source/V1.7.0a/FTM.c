...
#define tmr1_preload 40536 //10hz tmr1, 1:8 prescale
...
int8 temp_display_type;
int8 general_timer = 0, cat_command_timer = 0;
char temp_byte;
...
#endif
int8 res1, res2, dcs, state, old_state, band1, band2, band3, mem_channel, PLLband, fl_cnt, state1, state2, vfo;
...
int8 dummy_mode = 49;
int8 cat_tx_timeout = cat_tx_safety_timeout;
int8 cat_tx_timer = 0;
int8 cat_tx_long_timer = 0;
#endif
      ...
      if((update) || (jump))
      {
         ...
         IF(!gen_tx)
         {
            ...
            for (INT i = 0; i < 10; i++)
               {
                  valid = 0; PORTA = 6;
                  IF ( (storage_buffer[state] >= blacklist[i * 2]) && (storage_buffer[state] <= blacklist[ (i * 2) + 1])) {valid = 1; break;}
               }
            PORTA = old_PORTA;
